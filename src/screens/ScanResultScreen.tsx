...
export default function ScanResultScreen() {
  ...
  const config = {
    valid: {
      ...
      label: 'Ticket valide !', sub: 'Entrée autorisée', headerBg: 'from-green-900/30 to-[#0d0a1a]',
    },
    already_scanned: {
      icon: AlertTriangle, color: 'text-orange-400', bg: 'bg-orange-500/10', border: 'border-orange-500/30',
      label: 'Ticket déjà scanné', sub: 'Ce ticket a déjà été utilisé.', headerBg: 'from-orange-900/30 to-[#0d0a1a]',
    },
    invalid: {
        ...
        <div className="max-w-lg mx-auto flex flex-col gap-4">
          {ticket ? (
            <div className="bg-[#1e1640] rounded-2xl p-5 border border-white/[0.04]">
              <InfoRow icon={User} label="Nom" value={ticket.buyer_name || 'Inconnu'} />
              ...
              {result === 'already_scanned' && scanLog && (
                <>
                  ...
                  <InfoRow icon={User} label="Scanné par" value={scanLog.agent_name || '—'} />
                </>
              )}
            </div>
          ) : result === 'invalid' ? (
            <div className="bg-[#1e1640] rounded-2xl p-5 border border-white/[0.04]">
              ...
              <p className="text-gray-500 text-xs">Vérifiez le ticket et réessayez.</p>
            </div>
          ) : null}

          {result === 'valid' && ticket?.tombola_number && (
            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-2xl p-4">
              <p className="text-yellow-400 text-sm font-medium">Numéro tombola: {ticket.tombola_number}</p>
            </div>
          )}
        </div>
      </div>
